...
import { setCookie } from "../global/cokies"
...
export default function Register(){
    ...
    async function RegisterUser(): Promise<user | null>{
        try{
            const response = await fetch(`${import.meta.env.VITE_API_URL}RegistrationModels`,{
                method: "POST",
                headers:{
    ...
    return(
        ...
        <section className="flex justify-center items-center h-screen w-screen bg-gradient-to-b from-10% from-black via-neutral-500 via-70% to-gray-200">
            <div className="max-w-md w-full bg-transparent rounded p-6 space-y-4">
                ...
                <div>
                    <button className="w-full py-4 bg-orange-600 hover:bg-orange-700 rounded text-sm font-bold text-gray-50 transition duration-200" onClick={RegisterUser}>Register</button>
                </div>
                <div className="flex items-center justify-between">
                    <div>
                        <p className="mt-2">Already a user? <Link className="text-sm text-orange-600 hover:underline" to="/login">Login</Link></p>
                    </div>
                </div>
            </div>
        </section>
